...
class Rf():
    def __init__(self, 
        ...
        self.modem_sample_rate = self.modem.modem_sample_rate
        ...
        if rx_device == 'stdin':
            ...
            try:
                logging.debug(p.get_device_info_by_index(rx_dev))
            ...
            except OSError: # catch issue with non mono devices
                self.rx_mono = False
                logging.debug("Running in rx stereo")
                self.stream_rx = p.open(format=pyaudio.paInt16, 
                                channels=2,
                                rate=audio_sample_rate,
                                frames_per_buffer=2048,
        ...
        if tx_device == 'stdout':
            ...
            logging.debug(p.get_device_info_by_index(tx_dev))
            try:
        ...
        logging.debug(self.tx_mono )
        logging.debug(self.rx_mono )
    ...
    def rx(self):
        if type(self.stream_rx) == pyaudio.Stream:
            audio_sample = self.stream_rx.read(2048, exception_on_overflow = False) 
        else: # for stdin
            audio_sample = self.stream_rx.read(2048) # times 2 for 16bit samples
        if self.rx_mono == False:
            audio_sample = audioop.tomono(audio_sample, 2, 1, 0)
        (audio_sample, self.sample_state) = audioop.ratecv(audio_sample,2,1,self.audio_sample_rate, self.modem_sample_rate, self.sample_state)
